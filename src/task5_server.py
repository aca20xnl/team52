...
class Task3(object):
    ...
    def __init__(self):
        self.current_move = 'move towards wall'
    ...
    def action_server_launcher(self, goal):
            ...
            while success and not self.ctrl_c:
                if self.current_move == 'move towards wall':
                    self.vel.linear.x = 0.26
                    self.vel.angular.z = 0.6
                    self.vel_pub.publish(self.vel)
                
                elif self.current_move == 'turn right':
                    self.vel.linear.x = 0.0
                    self.vel.angular.z = -0.6
                    self.vel_pub.publish(self.vel)
                    
                elif self.current_move == 'go straight':
                    self.vel.linear.x = 0.26
                    self.vel.angular.z = 0.0
                    self.vel_pub.publish(self.vel)
                
                elif self.current_move == 'turn left':
                    self.vel.linear.x = 0.0
                    self.vel.angular.z = 0.6
                    self.vel_pub.publish(self.vel)
